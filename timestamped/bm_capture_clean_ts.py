...
"""
bm_capture.py — RS‑485 Bambu‑Bus Data Capture Tool (with Timestamping)

This script captures all non-zero data from a Bambu-Bus via an RS-485
serial adapter. It is optimized to discard inter-packet silence (0x00 bytes)
to keep file sizes manageable.

What it does:
1.  **Filtered Raw Capture:** Saves all non-zero bytes to a `.raw.bin` file.
2.  **Timestamp Log:** Creates a compact `.ts.bin` file containing high-precision
    timestamps for each burst of data, allowing for accurate synchronization.
3.  **Filtered Hex Log:** Creates a human-readable hex dump of the non-zero data.
...
class RotatingWriter:
    ...
    def write(self, data: bytes | str):
        if self.fh is None: raise IOError("Writer is closed.")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Bambu-Bus RS-485 data capture tool (read-only, with timestamping)", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--rotate-bytes", type=int, default=0, help="Rotate log files at this size (in bytes). 0 to disable.")
...
def open_serial_port(args: argparse.Namespace) -> serial.Serial:
    ...
    kwargs = {"port": args.port, "baudrate": args.baud, "bytesize": args.bytesize, "parity": parity_map[args.parity], "stopbits": args.stopbits, "timeout": args.timeout}
    if sys.platform.startswith("win"):
        try: return serial.Serial(**kwargs, exclusive=True)
        except TypeError: return serial.Serial(**kwargs)
    return serial.Serial(**kwargs)

def main() -> int:
    args = parse_args()
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_filename = f"{args.outfile}"
    ...
    ts_writer = RotatingWriter(base_filename, ".ts.bin", args.rotate_bytes, mode="wb")
    ...
    if hasattr(signal, "SIGTERM"): signal.signal(signal.SIGTERM, _handle_shutdown)

    try:
        ser = open_serial_port(args)
    except SerialException as e:
        print(f"ERROR: Could not open serial port '{args.port}': {e}", file=sys.stderr); return 1

    if not args.quiet:
        print(f"[bm_capture] Listening on {args.port} @ {args.baud} {args.bytesize}{args.parity}{args.stopbits}")
        print(f"[bm_capture] Writing non-zero data and timestamps to logs...")
    ...
    try:
        while not stop_flag["stop"]:
            if args.max_runtime > 0 and (time.time() - start_time) >= args.max_runtime:
                print("[bm_capture] Maximum runtime reached."); break
            try:
                chunk = ser.read(4096)
            except SerialException as e:
                print(f"\nERROR: Serial read failed: {e}", file=sys.stderr); break

            if chunk:
                non_zero_chunk = bytes(b for b in chunk if b != 0)
                if non_zero_chunk:
                    now = time.time()
                    current_offset = raw_writer.current_bytes
                    
                    # Write a timestamp record: 8-byte offset, 8-byte double timestamp
                    ts_record = struct.pack('<Qd', current_offset, now)
                    ts_writer.write(ts_record)
                    
                    raw_writer.write(non_zero_chunk)
                    hex_writer.write(f"{now:.6f} {non_zero_chunk.hex()}\n")

            if time.time() - last_flush_time > 2.0:
                raw_writer.flush(); hex_writer.flush(); ts_writer.flush()
                last_flush_time = time.time()

    finally:
        print("[bm_capture] Cleaning up...")
        if 'ser' in locals() and ser.is_open: ser.close()
        raw_writer.close(); hex_writer.close(); ts_writer.close()

    if not args.quiet: print("[bm_capture] Done.")
    return 0
